...
def my_transform3(image1, image2, label):
    ...
    label = tf.resized_crop(label, i, j, h, w, size)
    return image1, image2, label
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform == True:
            img1 = Image.open(img1_path).resize(size=(600,600),resample=PIL.Image.NEAREST)
            img2 = Image.open(img2_path).resize(size=(600,600),resample=PIL.Image.NEAREST)
            label = Image.open(label_path).resize(size=(600,600),resample=PIL.Image.NEAREST)
        else:
